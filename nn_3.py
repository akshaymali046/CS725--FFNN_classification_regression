...
class Optimizer(object):
  ...
  def step(self, weights, biases, delta_weights, delta_biases, lr):
    '''
    Parameters
    ----------
    ...
    weights = [w - lr * (m_hat / (np.sqrt(v_hat) + self.epsilon)) for w, m_hat, v_hat in zip(weights, m_hat_dw, v_hat_dw)] 
    biases = [b - lr * (m_hat / (np.sqrt(v_hat) + self.epsilon)) for b, m_hat, v_hat in zip(biases, m_hat_db, v_hat_db)]
...
def train(
  ...
  for e in range(max_epochs):
	  ...
	  for i in range(0, m, batch_size):
		  ...
		  weights_updated, biases_updated = optimizer.step(net.weights, net.biases, dW, db, lr)
	  ...
	  if e % 5 == 0: lr /= 5
	  print(e, rmse(dev_target, net(dev_input)))
...
class PCA(object):
  def __init__(self):
    self.u = np.zeros((90, 90))

  def __call__(self, data):
    data = (data - data.mean(axis = 0)) /  (data.max(axis = 0) - data.min(axis = 0)) 
    s, u = np.linalg.eig(np.cov(data.T))
    self.u = u[:, np.argsort(-s)]

  def predict(self, data):
    return data @ self.u[:NUM_FEATS].T
...
def read_data():
  ...
  pca(train_input)
  train_input = pca.predict(train_input)
  dev_input = pca.predict((dev_input - dev_input.mean(axis = 0)) / dev_input.std(axis = 0))
  test_input = pca.predict((test_input - test_input.mean(axis = 0)) / test_input.std(axis = 0))
